...
export class GeneratePdf {
    ...
    constructor(data: DocumentData, options?: Options) {
        ...
        options?.locale && i18n.setLocale(options.locale);
    }
    ...
    async generate(): Promise<void> {
        ...
        const whereToSavePath = this.options?.filePath ? path.resolve(this.options.filePath, fileName) : path.resolve(__dirname, fileName);

        var pdfDoc = this.printer.createPdfKitDocument(docDefinition, options);
        var a = pdfDoc.pipe(fs.createWriteStream(whereToSavePath));
        pdfDoc.end();
    }
